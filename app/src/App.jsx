  ...
  return (
    <div className="grid place-items-center w-screen h-screen bg-bg text-fg font-pp">
      <div className="flex flex-col  py-10 items-center h-screen w-screen sm:w-[min(100vw,80vh)]">
        ...
        <TimeControls {...props} />
        <Clock {...props} clockRef={clockRef} />
        <PlayControls {...props} clockRef={clockRef} />
      </div>
    </div>
  );
}
...
const Clock = (props) => {
  const { timeLeft, sessionLength, isSession, clockRef, playing } = props;

  let minutes = sessionLength < 60 || (timeLeft.getMinutes() < 60 && playing) ? timeLeft.getMinutes().toString().padStart(2, '0') : '60';
  let seconds = timeLeft.getSeconds().toString().padStart(2, '0');
  console.log(minutes, ":", seconds);
  return (
    <div ref={clockRef} id="clock" className="w-1/2 mt-5 py-10 sm:w-[calc(min(100vw,80vh)*0.5)] h-[calc(100vw*0.5)] sm:h-[calc(min(100vw,80vh)*0.5)] text-bg bg-fg rounded-full">
...
const PlayControls = (props) => {
  ...
  const toggle = () => {
    ...
    if (!prev) {
      ...
      setTimeLeft(new Date(Number(timeLeft) - 1000));
    }
  }
  // reset the timer
  const reset = () => {
    ...
    setIsBeeping(false);
    setIsSession(true);
    clockRef.current.style.backgroundColor = "#3B372E";
  }
    ...
    const playPause = () => {
      ...
      if (timeLeft.getSeconds() === 0 && timeLeft.getMinutes() === 0 && isSession) {
        ...
        if (newTime.getSeconds() === 0 && newTime.getMinutes() === 0) {
          ...
          clockRef.current.style.backgroundColor = "red";
        }
      }
    }
    intervalRef.current = setInterval(() => {
      if (playing && !isBeeping) {
        playPause();
      }
    }, 1000);
  ...
  return (
    <div className="flex gap-5 w-3/5 justify-center items-center mt-6">
      <audio id="beep" ref={audioRef} src={audioFile} onEnded={handleEnd} />
